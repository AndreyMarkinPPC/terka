...
from terka import exceptions, utils, views
...
class CommandHandler:
  ...
  def execute(
    self, command: str, entity: str, task_dict: dict | list[dict]
  ) -> None:
...
class SprintCommandHandlers:
  ...
  def create(
    cmd: commands.CreateSprint, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> int:
    if not cmd:
  ...
  def start(
    cmd: commands.StartSprint, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if not (
        existing_sprint := uow.tasks.get_by_id(entities.sprint.Sprint, cmd.id)
      ):
        raise exceptions.EntityNotFound(f'Sprint id {cmd.id} is not found')
      ...
      if existing_sprint.status.name == 'COMPLETED':
        raise exceptions.TerkaSprintCompleted('Cannot start completed sprint')
      uow.tasks.update(
      ...
      for sprint_task in existing_sprint.tasks:
        ...
        if task_params:
          ...
          uow.published_messages.append(commands.UpdateTask(**task_params))
        ...
        if sprint_task.story_points == 0 and context.get('ask-input'):
          ...
          try:
            ...
            uow.published_messages.append(
              commands.AddTask(
                id=task.id, sprint=existing_sprint.id, story_points=story_points
              )
            )
          except ValueError:
            print('[red]Provide number when specifying story points[/red]')
      logging.debug(f'Sprint started, context: {cmd}')

  @register(cmd=commands.UpdateSprint)
  def update(
    cmd: commands.UpdateSprint, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if not (
        existing_sprint := uow.tasks.get_by_id(entities.sprint.Sprint, cmd.id)
      ...
      uow.tasks.update(
        entities.sprint.Sprint, cmd.id, cmd.get_only_set_attributes()
      )
      uow.commit()

  @register(cmd=commands.CompleteSprint)
  def complete(
    cmd: commands.CompleteSprint,
    ...
    with bus.uow as uow:
      ...
      for sprint_task in existing_sprint.tasks:
        ...
        if task.status.name not in ('DONE', 'DELETED') and task.due_date:
  ...
  def delete(
    cmd: commands.DeleteSprint, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if not (
        existing_sprint := uow.tasks.get_by_id(entities.sprint.Sprint, cmd.id)
        ...
        raise exceptions.EntityNotFound(f'Sprint id {cmd.id} is not found')
      uow.tasks.update(entities.sprint.Sprint, cmd.id, {'status': 'DELETED'})
      uow.commit()
      bus.publisher.publish('Topic', events.SprintDeleted(cmd.id))

  @register(cmd=commands.ShowSprint)
  def show(
    cmd: commands.ShowSprint, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if not (
        existing_sprint := uow.tasks.get_by_id(entities.sprint.Sprint, cmd.id)
  ...
  def list(
    cmd: commands.ListSprint, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
  ...
  def note(
    cmd: commands.NoteSprint, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> int:
    if not cmd:
      cmd, context = templates.create_command_from_editor(
...
class SprintEventHandlers:
  def assign_story_points(
    event: events.SprintTaskStoryPointAssigned,
    bus: 'messagebus.MessageBus',
    context: dict = {},
  ) -> None:
    with bus.uow as uow:
      uow.tasks.update(
        entities.sprint.SprintTask,
        event.id,
...
class TaskCommandHandlers:
  ...
  def create(
    cmd: commands.CreateTask, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> int:
    if not cmd.name:
      cmd, context = templates.create_command_from_editor(
  ...
  def update(
    cmd: commands.UpdateTask, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if not (existing_task := uow.tasks.get_by_id(entities.task.Task, cmd.id)):
        raise exceptions.EntityNotFound(f'Task id {cmd.id} is not found')
      if not cmd:
        cmd, context = templates.create_command_from_editor(
      ...
      for f in cmd.__dataclass_fields__:
        ...
        if (f == 'due_date' and new_value != old_value) or (
          ...
          uow.published_messages.append(
            events.TaskUpdated(
              cmd.id, type=f.upper(), old_value=old_value, new_value=new_value
            )
          )
      ...
      if existing_task.status.name in ('DONE', 'DELETED') and status not in (
        'DONE',
        'DELETED',
      ):
  ...
  def collaborate(
    ...
    with bus.uow as uow:
      ...
      if not uow.tasks.list(
        entities.collaborator.TaskCollaborator,
        {'task': cmd.id, 'collaborator': user_id},
      ):
        uow.tasks.add(
          entities.collaborator.TaskCollaborator(
  ...
  def add(
    cmd: commands.AddTask, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    if (sprint := cmd.sprint) and (story_points := cmd.story_points):
      ...
      TaskCommandHandlers._add(
        commands.AddTask(cmd.id, sprint=sprint, story_points=story_points),
        bus,
        updated_context,
      )
      ...
      for field in cmd.__dataclass_fields__:
        if field_value := getattr(cmd, field):
          updated_context = {'entity_type': field, 'entity_id': field_value}
          if field == 'sprint':
            TaskCommandHandlers._add(
              commands.AddTask(cmd.id, sprint=field_value), bus, updated_context
            )
          if field == 'epic':
            TaskCommandHandlers._add(
              commands.AddTask(cmd.id, epic=field_value), bus, updated_context
            )
          if field == 'story':
            TaskCommandHandlers._add(
              commands.AddTask(cmd.id, story=field_value), bus, updated_context
            )

  def _add(
    cmd: commands.AddTask, bus: 'messagebus.MessageBus', context
  ) -> None:
    ...
    entity_task_type = getattr(entity_module, f'{entity_name.capitalize()}Task')
    ...
    with bus.uow as uow:
      ...
      if existing_entity_task := uow.tasks.list(entity_task_type, entity_dict):
        if not story_points:
          raise exceptions.TaskAddedToEntity(
            f'task {cmd.id} already added to ' f'{entity_name} {entity_id}'
          )
        else:
          uow.tasks.update(
            entity_task_type,
        ...
        if entity_name in ('story', 'epic'):
          ...
          if entity_task.story_points > existing_entity.remaining_capacity:
            raise exceptions.TerkaSprintOutOfCapacity(
              f'Sprint {entity_id} will overplanned '
              f'when task with {entity_task.story_points} is added'
            )
          uow.commit()
          if existing_task := uow.tasks.get_by_id(entities.task.Task, cmd.id):
            task_params = {}
            if existing_entity.status == entities.sprint.SprintStatus.ACTIVE:
              if existing_task.status.name == 'BACKLOG':
                task_params.update({'status': 'TODO'})
              if (
                not existing_task.due_date
                or existing_task.due_date > existing_entity.end_date
                or existing_task.due_date < existing_entity.start_date
              ):
                task_params.update({'due_date': existing_entity.end_date})
            if task_params:
  ...
  def assign(
    cmd: commands.AssignTask, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None: ...

  @register(cmd=commands.CompleteTask)
  def complete(
    cmd: commands.CompleteTask, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if not (existing_task := uow.tasks.get_by_id(entities.task.Task, cmd.id)):
        raise exceptions.EntityNotFound(f'Task id {cmd.id} is not found')
  ...
  def delete(
    cmd: commands.DeleteTask, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    updated_context = {}
    for field in cmd.__dataclass_fields__:
      if field == 'id':
      ...
      if field_value := getattr(cmd, field):
        ...
        if field == 'epic':
          TaskCommandHandlers._delete(
            commands.DeleteTask(cmd.id, epic=field_value), bus, updated_context
          )
        if field == 'story':
          TaskCommandHandlers._delete(
            commands.DeleteTask(cmd.id, story=field_value), bus, updated_context
          )
  ...
  def _delete(
    ...
    with bus.uow as uow:
      ...
      if 'entity_type' in context and 'entity_id' in context:
        ...
        entity_module = getattr(entities, entity_name)
        entity = getattr(entity_module, entity_name.capitalize())
        entity_task_type = getattr(
          entity_module, f'{entity_name.capitalize()}Task'
        )
        entity_dict = {'task': cmd.id}
        entity_dict[entity_name] = entity_id
        if not (existing_entity := uow.tasks.get_by_id(entity, entity_id)):
        ...
        if existing_entity_task := uow.tasks.list(
          ...
          uow.tasks.delete(entity_task_type, existing_entity_task[0].id)
          uow.commit()
          logging.debug(
  ...
  def comment(
    cmd: commands.CommentTask, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if text := cmd.text.strip():
        uow.tasks.add(entities.commentary.TaskCommentary(id=cmd.id, text=text))
        uow.tasks.update(
          entities.task.Task, cmd.id, {'modification_date': datetime.now()}
        )
        uow.commit()

  @register(cmd=commands.TrackTask)
  def track(
    cmd: commands.TrackTask, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if not (existing_task := uow.tasks.get_by_id(entities.task.Task, cmd.id)):
        raise exceptions.EntityNotFound(f'Task id {cmd.id} is not found')
  ...
  def tag(
    cmd: commands.TagTask, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if not (existing_task := uow.tasks.get_by_id(entities.task.Task, cmd.id)):
        raise exceptions.EntityNotFound(f'Task id {cmd.id} is not found')
      if not (
        existing_tag := uow.tasks.list(entities.tag.BaseTag, {'text': cmd.tag})
      ):
      ...
      if not uow.tasks.list(
        entities.tag.TaskTag, {'task': cmd.id, 'tag': tag_id}
      ):
  ...
  def _process_extra_args(id, context, uow):
    ...
    if collaborators := context.get('collaborators'):
      for collaborator_name in collaborators.split(','):
        uow.published_messages.append(
          commands.CollaborateTask(id=id, collaborator=collaborator_name)
        )
    if sprints := context.get('sprint'):
      story_points = context.get('story_points')
      for sprint in sprints.split(','):
        uow.published_messages.append(
          commands.AddTask(id=id, sprint=sprint, story_points=story_points or 0)
        )
    ...
    if stories := context.get('story'):
      for story in stories.split(','):
        uow.published_messages.append(commands.AddTask(id=id, story=story))
    if comment := context.get('comment'):
      uow.published_messages.append(commands.CommentTask(id=id, text=comment))
  ...
  def list(
    cmd: commands.ListTask, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
  ...
  def show(
    cmd: commands.ShowTask, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
...
class TaskEventHandlers:
  ...
  def created(
    event: events.TaskCreated, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
  ...
  def completed(
    ...
    with bus.uow as uow:
      ...
      uow.tasks.update(
        entities.task.Task, event.id, {'modification_date': datetime.now()}
      )
      uow.commit()

  @register(event=events.TaskUpdated)
  def updated(
    event: events.TaskUpdated, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      task_event = entities.event_history.TaskEvent(**asdict(event))
      uow.tasks.add(task_event)
      uow.tasks.update(
        entities.task.Task, event.task, {'modification_date': datetime.now()}
      )
      uow.commit()
  ...
  def deleted(
    event: events.TaskDeleted, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      task_event = entities.event_history.TaskEvent(
        task_id=event.id, type='STATUS', old_value='', new_value='DELETED'
      )
      uow.tasks.add(task_event)
      uow.tasks.update(
        entities.task.Task, event.d, {'modification_date': datetime.now()}
      )
      uow.commit()

  @register(event=events.TaskCommentAdded)
  def comment_added(
    event: events.TaskCommentAdded,
    bus: 'messagebus.MessageBus',
    context: dict = {},
  ) -> None:
    TaskCommandHandlers.comment(
      cmd=commands.CommentTask(**asdict(event)), bus=bus, context=context
    )
    with bus.uow as uow:
      uow.tasks.update(
        entities.task.Task, event.id, {'modification_date': datetime.now()}
  ...
  def collaborator_added(
    event: events.TaskCollaboratorAdded,
    bus: 'messagebus.MessageBus',
    context: dict = {},
  ) -> None:
    TaskCommandHandlers.collaborate(
      cmd=commands.CollaborateTask(**asdict(event)), bus=bus, context=context
    )

  @register(event=events.TaskHoursSubmitted)
  def hours_submitted(
    event: events.TaskHoursSubmitted,
    bus: 'messagebus.MessageBus',
    context: dict = {},
  ) -> None:
    TaskCommandHandlers.track(
      cmd=commands.TrackTask(id=event.id, hours=event.hours),
      bus=bus,
      context=context,
    )
  ...
  def added_to_epic(
    event: events.TaskAddedToEpic,
    bus: 'messagebus.MessageBus',
    context: dict = {},
  ) -> None:
    TaskCommandHandlers.add(
      cmd=commands.AddTask(id=event.id, epic=event.epic_id),
      bus=bus,
      context=context,
    )
  ...
  def added_to_sprint(
    event: events.TaskAddedToSprint,
    bus: 'messagebus.MessageBus',
    context: dict = {},
  ) -> None:
    TaskCommandHandlers.add(
      cmd=commands.AddTask(id=event.id, sprint=event.sprint_id),
      bus=bus,
      context=context,
    )
  ...
  def added_to_story(
    event: events.TaskAddedToStory,
    bus: 'messagebus.MessageBus',
    context: dict = {},
  ) -> None:
    TaskCommandHandlers.add(
      cmd=commands.AddTask(id=event.id, story=event.story_id),
      bus=bus,
      context=context,
    )
  ...
  def synced(
    event: events.TaskSynced, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if synced_task := uow.tasks.get_by_conditions(
        ...
        uow.tasks.update(
          asana.AsanaTask, synced_task[0].id, {'sync_date': event.sync_date}
        )
      else:
...
class ProjectCommandHandlers:
  ...
  def create(
    ...
    with bus.uow as uow:
      if not (
        existing_project := uow.tasks.get(entities.project.Project, cmd.name)
      ):
  ...
  def update(
    ...
    with bus.uow as uow:
      ...
      cmd = convert_workspace(cmd, bus)
      uow.tasks.update(
        entities.project.Project, project.id, cmd.get_only_set_attributes()
      )
      uow.commit()
      ProjectCommandHandlers._process_extra_args(project.id, context, uow)

  @register(cmd=commands.CompleteProject)
  def complete(
    cmd: commands.CompleteProject,
    bus: 'messagebus.MessageBus',
    context: dict = {},
  ) -> None:
    with bus.uow as uow:
      project = ProjectCommandHandlers._validate_project(cmd.id, uow)
      uow.tasks.update(
        entities.project.Project, project.id, {'status': 'COMPLETED'}
      )
      uow.commit()
  ...
  def delete(
    ...
    with bus.uow as uow:
      project = ProjectCommandHandlers._validate_project(cmd.id, uow)
      uow.tasks.update(
        entities.project.Project, project.id, {'status': 'DELETED'}
      )
      uow.commit()
  ...
  def comment(
    ...
    with bus.uow as uow:
      project = ProjectCommandHandlers._validate_project(cmd.id, uow)
      uow.tasks.add(
        entities.commentary.ProjectCommentary(id=project.id, text=cmd.text)
      )
      uow.commit()

  @register(cmd=commands.TagProject)
  def tag(
    cmd: commands.TagProject, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      project = ProjectCommandHandlers._validate_project(cmd.id, uow)
      ...
      if not (
        ...
        tag_id = existing_tag[0].id
      if not uow.tasks.list(
        entities.tag.ProjectTag, {'project': project.id, 'tag': tag_id}
      ):
        uow.tasks.add(entities.tag.ProjectTag(id=project.id, tag_id=tag_id))
        uow.commit()

  @register(cmd=commands.CollaborateProject)
  def collaborate(
    cmd: commands.CollaborateProject,
    bus: 'messagebus.MessageBus',
    context: dict = {},
  ) -> None:
    with bus.uow as uow:
      if not (
        existing_project := uow.tasks.get_by_id(
          entities.project.Project, cmd.id
        )
      ):
        raise exceptions.EntityNotFound(f'Project id {cmd.id} is not found')
      if not (
        existing_user := uow.tasks.list(
          entities.user.User, {'name': cmd.collaborator}
        )
      ...
      if not uow.tasks.list(
        ...
        {'project': cmd.id, 'collaborator': user_id},
      ):
        uow.tasks.add(
          entities.collaborator.ProjectCollaborator(
            id=cmd.id, collaborator_id=user_id
  ...
  def get(
    cmd: commands.GetProject, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> list[entities.project.Project]:
    with bus.uow as uow:
  ...
  def show(
    cmd: commands.ShowProject, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      project = ProjectCommandHandlers._validate_project(cmd.id, uow)
  ...
  def list(
    cmd: commands.ListProject, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      filter_options = utils.FilterOptions.from_kwargs(**context)
      with bus.uow as uow:
        if filter_options:
          projects = uow.tasks.get_by_conditions(
            entities.project.Project, filter_options.get_only_set_attributes()
          )
        else:
  ...
  def sync(
    cmd: commands.SyncProject, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if cmd.id:
        project = ProjectCommandHandlers._validate_project(cmd.id, uow)
    ...
    with bus.uow as uow:
      ...
      if not (
        ...
        asana_project = asana.AsanaProject(
          id=project.id, asana_project_id=cmd.external_project, sync_date=None
        )
  ...
  def _validate_project(id: str | int, uow) -> entities.project.Project:
    if isinstance(id, int) or id.isnumeric():
      if not (
        existing_project := uow.tasks.get_by_id(entities.project.Project, id)
      ):
        raise exceptions.EntityNotFound(f'Project id {id} is not found')
    else:
      if not (existing_project := uow.tasks.get(entities.project.Project, id)):
        raise exceptions.EntityNotFound(f'Project id {id} is not found')
  ...
  def _sync_project(uow, project: entities.project.Project):
    ...
    if last_sync_date := asana_project.sync_date:
      tasks = [
        ...
        if task.creation_date > last_sync_date
        or (task.modification_date and task.modification_date > last_sync_date)
      ]
    else:
      tasks = project.tasks
    if not tasks:
    ...
    uow.published_messages.append(
      events.ProjectSynced(project.id, asana_project_id, project_sync_date)
    )

  def _process_extra_args(id, context, uow):
    if tags := context.get('tags'):
      for tag in tags.split(','):
        uow.published_messages.append(commands.TagProject(id=id, tag=tag))
    if collaborators := context.get('collaborators'):
      for collaborator_name in collaborators.split(','):
        uow.published_messages.append(
          commands.CollaborateProject(id=id, collaborator=collaborator_name)
        )
    if comment := context.get('comment'):
      uow.published_messages.append(
...
class ProjectEventHandlers:
  ...
  def synced(
    event: events.ProjectSynced,
    bus: 'messagebus.MessageBus',
    context: dict = {},
  ) -> None:
    with bus.uow as uow:
      if synced_project := uow.tasks.get_by_conditions(
        asana.AsanaProject, {'asana_project_id': event.asana_project_id}
      ):
        uow.tasks.update(
          asana.AsanaProject,
          synced_project[0].id,
...
class EpicCommandHandlers:
  ...
  def create(
    cmd: commands.CreateEpic, bus: 'messagebus.MessageBus', context: dict = {}
  ):
    if not cmd.name:
      cmd, context = templates.create_command_from_editor(
  ...
  def update(
    cmd: commands.UpdateEpic, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if not (existing_epic := uow.tasks.get_by_id(entities.epic.Epic, cmd.id)):
        raise exceptions.EntityNotFound(f'Epic id {cmd.id} is not found')
      if not cmd:
        cmd, context = templates.create_command_from_editor(
      ...
      cmd.inject(bus.config)
      uow.tasks.update(
        entities.epic.Epic, cmd.id, cmd.get_only_set_attributes()
      )
      uow.commit()

  @register(cmd=commands.CompleteEpic)
  def complete(
    cmd: commands.CompleteEpic, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      uow.tasks.update(entities.project.Epic, cmd.id, {'status': 'COMPLETED'})
      uow.commit()
  ...
  def delete(
    cmd: commands.DeleteEpic, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
  ...
  def comment(
    cmd: commands.CommentEpic, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      uow.tasks.add(
  ...
  def add(
    cmd: commands.AddEpic, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if not (existing_epic := uow.tasks.get_by_id(entities.epic.Epic, cmd.id)):
        raise exceptions.EntityNotFound(f'Epic {cmd.id} is not found')
      if not uow.tasks.get_by_id(entities.sprint.Sprint, cmd.sprint):
        raise exceptions.EntityNotFound(f'Sprint {cmd.sprint} is not found')
  ...
  def list(
    cmd: commands.ListEpic, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
  ...
  def show(
    cmd: commands.ShowEpic, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
  ...
  def _process_extra_args(id, context, uow):
    if comment := context.get('comment'):
      uow.published_messages.append(commands.CommentEpic(id=id, text=comment))
...
class StoryCommandHandlers:
  ...
  def create(
    cmd: commands.CreateStory, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> int:
    if not cmd.name:
      cmd, context = templates.create_command_from_editor(
  ...
  def update(
    cmd: commands.UpdateStory, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if not (
        existing_story := uow.tasks.get_by_id(entities.story.Story, cmd.id)
      ):
        raise exceptions.EntityNotFound(f'Story id {cmd.id} is not found')
      if not cmd:
        cmd, context = templates.create_command_from_editor(
      ...
      cmd.inject(bus.config)
      uow.tasks.update(
        entities.story.Story, cmd.id, cmd.get_only_set_attributes()
      )
      uow.commit()

  @register(cmd=commands.CompleteStory)
  def complete(
    cmd: commands.CompleteStory,
    bus: 'messagebus.MessageBus',
    context: dict = {},
  ) -> None:
    with bus.uow as uow:
      uow.tasks.update(entities.project.Story, cmd.id, {'status': 'COMPLETED'})
      uow.commit()
  ...
  def delete(
    cmd: commands.DeleteStory, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
  ...
  def comment(
    cmd: commands.CommentStory, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      uow.tasks.add(
  ...
  def add(
    cmd: commands.AddStory, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if not (
        existing_story := uow.tasks.get_by_id(entities.story.Story, cmd.id)
  ...
  def list(
    cmd: commands.ListStory, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
  ...
  def show(
    cmd: commands.ShowStory, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
  ...
  def _process_extra_args(id, context, uow):
    if comment := context.get('comment'):
      uow.published_messages.append(commands.CommentStory(id=id, text=comment))
...
class TagCommandHandlers:
  ...
  def create(
    cmd: commands.CreateTag, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if not uow.tasks.get_by_conditions(
        entities.tag.BaseTag, {'text': cmd.text}
      ):
  ...
  def delete(
    cmd: commands.DeleteTag, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      if tags := uow.tasks.get_by_conditions(
        entities.tag.BaseTag, {'text': cmd.text}
      ):
  ...
  def list(
    cmd: commands.ListTag, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
...
class UserCommandHandlers:
  ...
  def create(
    cmd: commands.CreateUser, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
  ...
  def list(
    cmd: commands.ListUser, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
  ...
  def connect(
    cmd: commands.ConnectUser, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
      user = UserCommandHandlers._validate_user(cmd.id, uow)
      if not (asana_user := uow.tasks.get_by_id(asana.AsanaUser, user.id)):
        asana_user = asana.AsanaUser(
          id=user.id, asana_user_id=cmd.external_user
        )
  ...
  def _validate_user(id: str | int, uow) -> entities.user.User:
    if isinstance(id, int) or id.isnumeric():
      if not (existing_user := uow.tasks.get_by_id(entities.user.User, id)):
...
class NoteCommandHandlers:
  ...
  def show(
    cmd: commands.ShowNote, bus: 'messagebus.MessageBus', context: dict = {}
  ) -> None:
    with bus.uow as uow:
...
def convert_project(
  cmd: commands.Command, bus: 'messagebus.MessageBus', context: dict = {}
  ...
  if project_name.isnumeric():
    ...
    return cmd
  if not (
    existing_project := bus.uow.tasks.get(
      entities.project.Project, project_name
    )
  ):
    answer = Confirm.ask(
    ...
    project_id = ProjectCommandHandlers.create(
      cmd=commands.CreateProject(name=project_name), bus=bus, context=context
    )
...
def convert_workspace(
  ...
  if not (
    existing_workspace := bus.uow.tasks.get(
      entities.workspace.Workspace, workspace
    )
  ):
    print(
      f'Creating new workspace: {workspace}. ' 'Do you want to continue (Y/n)?'
    )
    answer = input()
    while answer.lower() != 'y':
      ...
      project_name = input()
      print(
        f'Creating new workspace: {workspace}. '
        'Do you want to continue (Y/n)?'
      )
      answer = input()
    workspace_id = WorkspaceCommandHandlers.create(
      cmd=commands.CreateWorkspace(name=workspace), bus=bus, context=context
    )
...
def convert_user(
  ...
  try:
    ...
    ...
  if not (existing_user := bus.uow.tasks.get(entities.user.User, user_value)):
