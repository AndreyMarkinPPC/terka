...
from terka.domain import commands, entities
  ...
  def on_data_table_cell_selected(self, event: DataTable.CellSelected):
    ...
    with self.bus.uow as uow:
      if 'task' in self.selected_data_table:
        ...
        self.query_one(components.Description).text = task_obj.description
        self.query_one(components.Status).value = task_obj.status.name
        self.query_one(components.Priority).value = task_obj.priority.name
        self.query_one(components.Project).value = task_obj.project_name
      ...
      if 'epic' in self.selected_data_table:
        ...
        self.query_one(components.Description).text = epic_obj.description
      if 'story' in self.selected_data_table:
        story_obj = uow.tasks.get_by_id(entities.story.Story, selected_id)
        self.query_one(components.Title).text = story_obj.name
        self.query_one(components.Description).text = story_obj.description


class PopupsMixin:
  def __init__(self): ...

  def _process_commands_chain(
    self, result: tuple[commands.Command, ...], project: str | None = None
  ) -> int | None:
  ...
  def action_task_complete(self) -> None:
    self.push_screen(components.TaskComplete(), self.task_complete_callback)

  def task_complete_callback(
    self,
    result: tuple[
      commands.CompleteTask, commands.CommentTask, commands.TrackTask
    ],
  ):
  ...
  def task_edit_callback(
    self, result: tuple[commands.UpdateTask, commands.CommentTask]
  ):
    self._process_commands_chain(result)
  ...
  def task_add_callback(self, result: commands.AddTask):
    if epic := result.epic:
      try:
        ...
        self.notify(f'Task: {self.selected_task} is added to epic {epic}!')
    ...
    if sprint := result.sprint:
      ...
      try:
        ...
        self.notify(
          f'Cannot add task to completed sprint {sprint} !', severity='error'
        )
      if not story_points:
        self.notify(f'Task: {self.selected_task} is added to sprint {sprint}!')
      else:
        self.notify(
    ...
    if story := result.story:
      try:
        ...
        self.notify(f'Task: {self.selected_task} is added to story {story}!')
      except exceptions.EntityNotFound:
  ...
  def action_task_delete(self) -> None:
    if self.selected_column == 'due_date':
      self.push_screen(
        components.TaskDeleteDueDate(), self.task_delete_due_date_callback
      )
    else:
      self.push_screen(components.TaskDelete(), self.task_delete_callback)

  def task_delete_callback(
    self,
    result: tuple[
      commands.CompleteTask, commands.CommentTask, commands.TrackTask
    ],
    ...
    if is_sprint:
      ...
      self.notify(f'Task: {self.selected_task} is deleted!', severity='warning')

  def action_task_update_context(self) -> None:
    if self.selected_column == 'status':
      self.push_screen(
        components.TaskStatusEdit(), self.task_update_status_callback
      )
    if self.selected_column == 'priority':
      self.push_screen(
        components.TaskPriorityEdit(), self.task_update_priority_callback
      )
    if self.selected_column == 'story_points':
      self.push_screen(
        components.TaskStoryPointsEdit(), self.task_update_story_points_callback
      )
    if self.selected_column == 'time_spent':
      self.push_screen(
        components.TaskHoursSubmitted(), self.task_update_hours_callback
      )
    if self.selected_column == 'tags':
      self.push_screen(components.TaskTagEdit(), self.task_update_tag_callback)
    if self.selected_column == 'collaborators':
      self.push_screen(
        components.TaskCollaboratorEdit(),
        self.task_update_collaborator_callback,
      )

  def task_update_status_callback(self, result: str):
    if result:
      self.bus.handle(commands.UpdateTask(id=self.selected_task, status=result))
  ...
  def task_update_priority_callback(self, result: str):
    if result:
      ...
      self.notify(f'Task: {self.selected_task} priority updated to {result}!')

  def task_update_story_points_callback(self, result: str):
    if result:
      self.bus.handle(
        commands.AddTask(
          id=self.selected_task, sprint=self.sprint_id, story_points=result
        )
      )
      self.notify(
  ...
  def task_update_hours_callback(self, result: str):
    if result:
      self.bus.handle(commands.TrackTask(id=self.selected_task, hours=result))
  ...
  def task_update_collaborator_callback(self, result: str):
    if result:
      self.bus.handle(
        commands.CollaborateTask(id=self.selected_task, collaborator=result)
      )
      self.notify(f'Added collaborator {result} for task {self.selected_task}!')
...
class TerkaTask(App):
  ...
  def compose(self) -> ComposeResult:
    ...
    yield Static(
      f'Project: [bold]{self.entity.project_name}[/bold]', classes='transp'
    )
    yield Static(
      f'Status: [bold]{self.entity.status.name}[/bold]', classes='transp'
    )
    yield Static(
      f'Priority: [bold]{self.entity.priority.name}[/bold]', classes='transp'
    )
    created_days_ago = (datetime.now() - self.entity.creation_date).days
    creation_message = (
      f'{created_days_ago} days ago' if created_days_ago > 1 else 'today'
    )
    ...
    if self.entity.is_completed:
      ...
      if completion_date:
        ...
        completion_message = (
          f'{completed_days_ago} days ago'
          if completed_days_ago > 1
          else 'today'
        )
        yield Static(
    ...
    if collaborators := self.entity.collaborators:
      ...
      yield Static(
        f'Collaborators: [bold]{collaborators}[/bold]', classes='transp'
      )
    else:
    ...
    if description_message:
      ...
      yield Static(
        '[italic]No description...[/italic]', classes='body', id='desc'
      )
  ...
  def compose(self) -> ComposeResult:
    ...
    with TabbedContent(initial='tasks'):
      with TabPane('Backlog', id='backlog'):
        yield Button(
          '+Task', id='new_task', variant='success', classes='new_entity'
        )
        ...
        for task in sorted(
          self.entity.backlog_tasks, key=lambda x: x.id, reverse=True
        ):
          table.add_row(
            color_task_id(task),
      ...
      with TabPane('Open Tasks', id='tasks'):
        ...
        for column in (
          ...
          table.add_column(column, key=column)
        for task in sorted(
          self.entity.open_tasks, key=lambda x: x.status.name, reverse=True
        ):
          table.add_row(
            color_task_id(task),
            add_comment_count(task),
      ...
      with TabPane('Epics', id='epics'):
        yield Button(
          '+Epic', id='new_epic', variant='success', classes='new_entity'
        )
        ...
        for epic in sorted(
          self.entity.epics, key=lambda x: len(x.tasks), reverse=True
        ):
          if epic.open_tasks:
            table.add_row(
              str(epic.id),
      ...
      with TabPane('Stories', id='stories'):
        yield Button(
          '+Story', id='new_story', variant='success', classes='new_entity'
        )
        ...
        for story in sorted(
          self.entity.stories, key=lambda x: len(x.tasks), reverse=True
        ):
          if story.open_tasks:
            table.add_row(
              str(story.id),
      ...
      with TabPane('Notes', id='notes'):
        yield Button(
          '+Note', id='new_note', variant='success', classes='new_entity'
        )
        table = DataTable(id='project_notes_table')
        for column in ('id', 'date', 'name'):
          table.add_column(column, key=f'note_{column}')
        for note in sorted(
          self.entity.notes, key=lambda x: x.date, reverse=True
        ):
          table.add_row(
            str(note.id), note.date.strftime('%Y-%m-%d'), note.name, key=note.id
          )
        yield table
      ...
      with TabPane('Overview', id='overview'):
        ...
        for name, value in sorted(
          ...
          sorted_collaborators += (
            f'  * {name}: {Formatter.format_time_spent(value)} \n'
          )
        yield Markdown(f"""
# Project details:
...
{sorted_collaborators}
  ...
  def task_new_callback(self, result: list[commands.Command]):
    object_id = self._process_commands_chain(result, project=self.project_id)
    ...
    with TabbedContent(initial='tasks'):
      with TabPane('Tasks', id='tasks'):
        ...
        for column in (
          ...
          table.add_column(column, key=column)
        for task in sorted(
          self.entity.open_tasks, key=lambda x: x.status.value, reverse=True
        ):
          table.add_row(
            color_task_id(task),
            add_comment_count(task),
      ...
      with TabPane('Completed Tasks', id='completed_tasks'):
        ...
        for task in sorted(
          ...
          table.add_row(
            ...
            task.tags_string,
            task.collaborators_string,
            Formatter.format_time_spent(round(task.story_points * 60)),
            Formatter.format_time_spent(task.total_time_spent),
            key=task.id,
          )
        yield table
      ...
      with TabPane('Time', id='time'):
        ...
        if not (sprint_time := self.entity.daily_time_entries_hours()):
          ...
          all_workspace_time = workspace.daily_time_entries_hours(
            start_date=self.entity.start_date, end_date=self.entity.end_date
          )
          non_sprint_times = [
      ...
      with TabPane('Overview', id='overview'):
        ...
        sorted_projects = ''
        for name, value in sorted(
          project_split.items(), key=lambda x: x[1], reverse=True
        ):
          sorted_projects += (
            f'  * {name}: {Formatter.format_time_spent(value)} \n'
          )
  ...
  def compose(self) -> ComposeResult:
    ...
    with TabbedContent(initial='tasks'):
      ...
      with TabPane('Time', id='time'):
        plotext = PlotextPlot(classes='plotext')
        plt = plotext.plt
        if not (composite_time := self.entity.daily_time_entries_hours()):
          yield Static('No time tracked')
        else:
          plt.date_form('Y-m-d')
          plt.title(
          ...
          plt.stacked_bar(composite_time.keys(), composite_time.values())
          yield plotext
...
def get_times(
  dates: list[str], time_entries: list[dict[str, str | int]]
) -> list[float]:
  times: list[float] = []
  for date in dates:
    value_for_date = 0
