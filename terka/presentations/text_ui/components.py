...
class NewTask(TerkaModalScreen):
  def compose(self) -> ComposeResult:
    yield Grid(
      ...
      Input(
        placeholder='Description', id='description', classes='description-input'
      ),
      Input(placeholder='Project', id='project', classes='project-input'),
      Input(placeholder='Assignee', id='assignee', classes='assignee-input'),
      Select(
        (
          (line, line)
          for line in [
      ...
      Input(
        placeholder='story_points', id='story_points', validators=[Number()]
      ),
  ...
  def on_button_pressed(self, event: Button.Pressed) -> None:
    if event.button.id == 'yes':
      ...
      story_points = self.query_one('#story_points', Input)
...
class NewEpic(TerkaModalScreen):
  ...
  def on_button_pressed(self, event: Button.Pressed) -> None:
    if event.button.id == 'yes':
      ...
      description = self.query_one('#description', Input)
      self.dismiss(
        commands.CreateEpic(name=name.value, description=description.value)
      )
    else:
      self.app.pop_screen()
...
class NewStory(TerkaModalScreen):
  ...
  def on_button_pressed(self, event: Button.Pressed) -> None:
    if event.button.id == 'yes':
      ...
      description = self.query_one('#description', Input)
      self.dismiss(
        commands.CreateStory(name=name.value, description=description.value)
      )
    else:
      self.app.pop_screen()
...
class TaskAdd(TerkaModalScreen):
  def compose(self) -> ComposeResult:
    yield Grid(
      ...
      Input(
        placeholder='Story points', id='story-points', validators=[Number()]
      ),
...
class TaskEdit(TerkaModalScreen):
  def compose(self) -> ComposeResult:
    yield Grid(
      ...
      Input(placeholder='Add time spent', validators=[Number()], id='hours'),
  ...
  def on_button_pressed(self, event: Button.Pressed) -> None:
    if event.button.id == 'yes':
      ...
      commands_chain.append(commands.CommentTask(id=None, text=comment.value))
      commands_chain.append(commands.TrackTask(id=None, hours=hours.value))
...
class TaskHoursSubmitted(ModalScreen[str]):
  def compose(self) -> ComposeResult:
    yield Grid(
      ...
      Input(
        placeholder='Time spent in minutes', id='hours', validators=[Number()]
      ),
      Button('Confim', id='yes'),
      Button('Cancel', id='no'),
      id='dialog',
